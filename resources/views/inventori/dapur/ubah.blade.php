    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <form action="{{url('/inventori/ubahdapur',$dapur->id)}}" method="post"> 
                {{ csrf_field() }}                                   
                <label class="col-md-4 control-label">Nama Bahan Dapur</label>
                <input type="text" class="form-control" name="name" value="{{$dapur->name}}" required>
                <label class="col-md-4 control-label">Jenis Bahan Dapur</label>
                <input type="text" class="form-control" name="type" value="{{$dapur->type}}" required>
                <label class="col-md-4 control-label">Jumlah Bahan Dapur</label>
                <input type="text" class="form-control" name="quantity" value="{{$dapur->quantity}}" required>
                <label class="col-md-4 control-label">Deskripsi</label>    
                <textarea class="form-control" placeholder="Berikan deskripsi disini" rows="3" name="deskripsi" required>
                {{$dapur->deskripsi}}
                </textarea>                                
                <input type="submit" class="btn btn-danger" value="Ubah Bahan Dapur"/>
                </form>                                
                </div>
                </div>
                </div>
